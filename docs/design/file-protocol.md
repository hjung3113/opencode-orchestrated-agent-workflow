...
the active graph, Task States, Runtime Binding States, effective Run Policy and
budgets, and an ordered transition log. Every other authoritative Artifact is
immutable. Prose notes
...
- Packet: role, `workflow_definition`, objective, acceptance criteria, allowed and forbidden
  ...
  escalation condition. `admitted_commands` are exact argv arrays with a
...
- Runtime Observation: adapter-observed OpenCode identity, events, complete
  ...
  Runtime Exit Reason for one Attempt. Each successful External Read preserves the
...
- Outcome Record: a Kernel-published checkpoint of one kind: Receipt, Material
  ...
  decisions, graph, Tasks, Runtime Binding States, effective Run Policy, current
  Output Snapshot, required Promotion, verification, and known limitations.
